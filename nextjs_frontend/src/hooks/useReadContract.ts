    ...
    async (address: `0x${string}`) => {
      ...
      try {
        ...
        */
        ...
        const catPromises = offers.map(async (id) => {
          if (!catInstance.read.getCat) return;
          const array = (await catInstance.read.getCat?.([Number(id)])) as bigint[];
          return mapArrayToCatObject(array);
        });
        const catDatas = (await Promise.all(catPromises));

        
        const catSellerPromise = catDatas.map(async (cat) => {
          if (!marketplaceInstance.read.getOffer) return;
          const _catId = parseInt(cat.indexId);
          // console.log(_catId);
          const array = (await marketplaceInstance.read.getOffer?.([Number(_catId)]));
          return mapArrayToOfferObject(array);
        });

        const catOffersDatas = (await Promise.all(catSellerPromise));
        ...
        for (let i = 0; i < catDatas.length; i ++) {
          ...
          catOffersDatas[i].ownOffer = catOffersDatas[i].seller === address;
          catOffers.push({ catData:catDatas[i], marketData: catOffersDatas[i] });
        }
        ...
        offers.map((catId) => {
          if (!catInstance.read.getCat) return;
          const array = (await catInstance.read.getCat?.([Number(catId)])) as bigint[];
          return mapArrayToCatObject(array);
        })
